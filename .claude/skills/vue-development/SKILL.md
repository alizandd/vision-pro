...
- **Vue 3.5** (stable) with the **Composition API** + `<script setup>` and **TypeScript**. Vite 7 build. **Vue 3.6 is still beta** — its headline **Vapor Mode** (compile-time, vDOM-skipping rendering) and the alien-signals reactivity rework are **opt-in and not production-default yet**; stable is expected around Q4 2026. Adopt Vapor per-component only with a measured perf reason.
- **Nuxt 4.x** for SSR/SSG/universal apps (Nitro engine, file-based routing, auto-imports); 4.4 brought Vue Router 5. **Nuxt 5 (Nitro 3) has shipped** — treat it as the target for greenfield once the module ecosystem you depend on supports it, not as an in-place upgrade mid-project. Plain Vue + Vite + Vue Router for SPAs.
