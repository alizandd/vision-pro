...
description: Emil Kowalski's design-engineering craft for UI polish and animation — easing curves, durations, springs, gestures, clip-path, transform/transform-origin, perceived performance, reduced-motion, and the invisible details that make interfaces feel right. Use when building or polishing web UI motion/animations, deciding whether/how something should animate, choosing easing or timing, or making a component feel great. For strictly reviewing existing motion against the bar use `review-animations`; for a codebase-wide motion audit-and-plan use `improve-animations`; for naming an effect use `animation-vocabulary`.
---
